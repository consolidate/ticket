...
use Consolidate\Ticket\Importer\Importer;

class ImporterTest extends PHPUnit_Framework_TestCase
{
    public function testImport() {
        $imap = new Imap(new testMailTransport2('test', 'test', 'test'));

        $importer = new Importer();
        $importer->addSource($imap);
        $tickets = $importer->process();

        $this->assertCount(1, $tickets);

        $events = $tickets[0]->getTimeline();
...
class testMailTransport2 implements TransportInterface {
